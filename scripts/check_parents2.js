...
console.log('\nAll modules with name starting @minecraft/server-bindings:');
for (const mod of modules) {
  if (mod.name && mod.name.startsWith('@minecraft/server-bindings')) {
    console.log(' ->', mod.name, mod.version, mod.minecraft_version, 'hasParentModule=', !!mod.parentModule, 'keys=', Object.keys(mod).slice(0,8).join(', '));
  }
}
